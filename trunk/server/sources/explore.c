...
t_map	*get_case_front(t_env *e, t_players *player, int level, int cx)
{
  ...
  x = (player->direction == UP) ? player->posx - level + cx : 0;
  y = (player->direction == UP) ? player->posy - level : 0;
  ...
  x = (player->direction == LEFT) ? player->posx + level : x;
  y = (player->direction == LEFT) ? player->posy - level + cx : y;
...
void	explore(t_env *e, t_players *player)
{
  ...
  write_case_content(player, &(e->map[player->posx][player->posy]), i);
  rb_write(player->wr_rb, ",", 1);
